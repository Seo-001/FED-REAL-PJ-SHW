...
function loadFn() {
  ...
  ***************************/
    ...
    ****************************************/
    ...
    /*  let aaa= aph2.getBoundingClientRect();
    console.log("aaa", aaa); */
    ...
    window.addEventListener("scroll", () => {
        ...
        //     if (tittop < hv && tittop > 100) {
        //         let mytxt = ele.querySelectorAll("span");
        //         mytxt.forEach((tele, tidx) => {
        //             tele.style.display = "inline-block";
        //             // tele.style.transform = `translate(-1000%,${tidx * 100}%)`;
        //             tele.style.transform = `translate(-1000%,${tidx * 100}%)`;
        //             tele.style.transition = `1s`;
        //             tele.style.lineHeight = ``;
        //             console.log("ele", ele);
        //         });
        //
        //     }
    ...
    **********************************/
    ...
    // 팁
    // 숫자로 된 이름은 없다. 숫자로 이름 못적음
    // 여러가지 정보를 적을 때는 배열을 추천

    const a = [
        "https://www.mk.co.kr/news/economy/7784222",
        "https://www.etoday.co.kr/news/view/1343558",
        "https://www.etoday.co.kr/news/view/1377898",
    ];

    const artexp = [
        [
            "[트렌드] 작은 공방서 시작한 올가니카…`클렌즈주스`로 날다",
            "비가열식 주스 국내 첫 개발…디톡스 열풍에 초고속 성장, 곡물·간편식 업체 인수로 종합 푸드기업 거듭나, 이마트·코스트코·스타벅스등 초대형 유통 채널 진출",
        ],
        [
            "국내 최초 클렌즈주스’ 올가니카, 저스트주스 안성생산센터 추가 증설 부지 매입",
            "폭발적 수요 발맞춰 인접 8600㎡ 대지에 신규 생산센터 건립",
        ],
        [
            "간편하게 건강 챙기는 클렌즈주스 인기… 올가니카, 올해 매출 300억 기대",
            "자사 온라인 몰 및 스타벅스, CU, 커피빈, 현대백화점, 신세계백화점 판매",
        ],
    ];
    ...
    /**************************
        ...
        이벤트 :가로 스크롤 
         ...
         [
            "WWF-Korea 파트너십",
            "세계자연기금 WWF (World Wide Fund For Nature) 세계자연기금은 자연 보전을 위해 설립된국제 비정부 기구",
        ],
    **************************/
    ...
    const des = [
        ...
        [
            "한국의 해양보전활동",
            "한국을 모범적인 지속가능한 어업국가로 만들기 위해 정부, 민간부문, 학계 및 NGO 단체와의 협력",
        ],
        [
            "생태발자국 조사 및 감축",
            "우리의 생활을 유지하기 위해 지구에 남기는 생태발자국을 조사하고 이를 감축하기 위한 방안 제안",
        ],
    ...
    // const indes = mvbx.querySelectorAll(".spdes");

    // console.log(des[0][0]);

    // indes.forEach((ele,idx)=>makeexplain(ele,idx));

    // 여기 있으니 휠 작동 안됨, ul 변수 적용 다음에 있어야 하지 않나?
    // makeList();
    /******************************* 
    ...
    *******************************/
    ...
    function movePage() {
        ...
        //  대상: const mvbx = sticky.querySelector("ul");
        //    const mvbx = document.querySelector(".bgcp ul");
    ...
    함수: makeList
    기능: 이미지 리스트 넣기 
     <img src="./img/sp01.png" alt="한국의 해양보전활동 이미지" />
    var i = 0; i < mini.length; i++)
    ***************************/
    function makeList() {
        let hcode = "<ul>";
        for (let i = 0; i < 5; i++) {
