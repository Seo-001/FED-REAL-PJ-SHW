...
const title = [
    ['Real Gone (From "Cars"/Soundtrack Version)', 'Life is a Highway (From "Cars"/Soundtrack Version)', 'Our Town (From "Cars"/Soundtrack Version)', "Sh-Boom (『カーズ』より)", 'Goodbye (From "Cars"/Score)', 'Behind the Clouds (From "Cars"/Soundtrack Version)'],
    ['You Might Think (From "Cars 2"/Soundtrack Version)', 'Collision of Worlds (From "Cars 2"/Soundtrack Version)', "Nobody's Fool (From 'Cars 2'/Soundtrack Version)", 'Going To The Backup Plan (From "Cars 2"/Score)', 'The Other Shoot (From "Cars 2"/Score)', 'The Radiator Springs Gran Prix (From "Cars 2"/Score)'],
    ["Storm's Winning Streak (From 'Cars 3'/Score)", "Fireball Beach (From 'Cars 3'/Score)", "Smokey Starts Training", "Drip Pan (From 'Cars 3'/Score)", "McQueen's Wild Ride (From 'Cars 3'/Score)", "Temple of Rust-eze (From 'Cars 3'/Score)"],
];
...
function setBan() {
    // 추가
    const arrow = document.querySelectorAll(".arrow_nav");
    ...
    bannerWrap.forEach((ele, idx) => {
        ele.innerHTML = `<img src="../TEAM_CAR/image/c${idx + 1}_teaser_poster.jpg" alt="포스터${idx + 1}이미지">`;
    ...
    // 추가
    arrow.forEach((ele) => (ele.title = `누르세요`));
    /*querySelectorAll  을 쓰면 for 문 돌려야 한다.
    arrow.title = `누르세요`; 이렇게 쓰면 안된다. 
    window.addEventListener("DOMContentLoaded", setBan);  을 잘 보고 
    setBan()  { }  안으로 보냄 
    function setBan() {
    // 추가 
    const arrow = document.querySelectorAll(".arrow_nav"); 
